// ═══════════════════════════════════════════════════════════════════════════
// 🚀 RATING NODE ULTIMATE V2 - TIERED POSITION SIZING
// ═══════════════════════════════════════════════════════════════════════════
//
...
// ✅ 🆕 V2: TIERED position sizing (only ELITE setups get max capital!)
// ✅ Small account optimization (aggressive leverage-ready)
// ✅ BUY & SELL (SHORT) signals with EXTREME conviction
//
// 🆕 V2 Changes:
// - Position sizing based on conviction TIERS (EXTREME=75, HIGH=60, MEDIUM=45)
// - VP quality MULTIPLIERS (not flat bonus): GOLDEN=1.20, EXCELLENT=1.13
// - Scenario MULTIPLIERS (not flat USDT): ALT_DECOUPLING=1.08, PEAK_ALT=1.10
// - Result: Only EXTREME + EXCELLENT/GOLDEN get 85-90 USDT
//
...
//===============================================================================
// 🔧 V2 POSITION SIZING - TIERED SYSTEM (30-90 USDT)
//===============================================================================

const POSITION_SIZING = {
  min_trade_size_usdt: 30,        // Minimum
  max_trade_size_usdt: 90,        // Maximum

  // 🆕 V2: Conviction-based BASE tiers (not multipliers!)
  conviction_base: {
    "EXTREME": 75,                // Elite setups start at 75 USDT
    "HIGH": 60,                   // Strong setups start at 60 USDT
    "MEDIUM": 45,                 // Decent setups start at 45 USDT
    "LOW": 30                     // Marginal setups at minimum
  },

  // 🆕 V2: VP quality MULTIPLIERS (not flat USDT bonus!)
  vp_quality_multiplier: {
    "GOLDEN": 1.20,               // 75 × 1.20 = 90 (max!)
    "EXCELLENT": 1.13,            // 75 × 1.13 = 84.75 ≈ 85
    "GOOD": 1.0,                  // No change
    "MODERATE": 0.87              // 75 × 0.87 = 65.25 ≈ 65 (penalty)
  },

  // 🆕 V2: Volatility MULTIPLIERS (refined)
  volatility_multiplier: {
    "EXTREME": 0.75,              // Reduce 25% in extreme vol
    "HIGH": 0.90,                 // Reduce 10%
    "MEDIUM": 1.0,                // Normal
    "LOW": 1.10                   // Increase 10% in low vol
  },

  // 🆕 V2: Market scenario MULTIPLIERS (not flat USDT!)
  scenario_multiplier: {
    "PEAK_ALT_SEASON": 1.10,      // +10% max (75 → 82.5, 90 stays 90)
    ...
    "CAPITULATION": 0.70,         // -30% (defensive mode)
...
console.log(`   💵 Position Range: ${POSITION_SIZING.min_trade_size_usdt}-${POSITION_SIZING.max_trade_size_usdt} USDT (TIERED)`);
console.log(`   🎯 Goal: ELITE setups get MAX capital, others scaled proportionally`);
...
//===============================================================================
// 🆕 V2 TIERED POSITION SIZING (PROPORTIONAL, NOT FLAT BOOSTS!)
//===============================================================================
...
function calculatePositionSizeUSDT(coin, conviction, vpQuality, marketState, marketScenario, sectorLeadership) {
  ...
  let size_usdt = POSITION_SIZING.conviction_base[conviction] || 45;

  // 2️⃣ Apply VP quality MULTIPLIER (not flat bonus!)
  const vpMult = POSITION_SIZING.vp_quality_multiplier[vpQuality] || 1.0;
  size_usdt *= vpMult;

  // 3️⃣ Apply volatility MULTIPLIER
  const volMult = POSITION_SIZING.volatility_multiplier[marketState.volatility_regime] || 1.0;
  size_usdt *= volMult;

  // 4️⃣ Apply scenario MULTIPLIER (not flat USDT!)
  const scenario = marketScenario?.scenario || "NEUTRAL";
  const scenarioMult = POSITION_SIZING.scenario_multiplier[scenario] || 1.0;
  size_usdt *= scenarioMult;

  // 5️⃣ Sector leader small boost (3% if leader)
